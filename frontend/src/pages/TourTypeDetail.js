...
const TourTypeDetail = () => {
    ...
    const decodedTourTypeName = tourTypeName.replace(/-/g, ' ');
    ...
    console.log(storedDeparturePlace)

    useEffect(() => {
        axiosInstance.get(`http://localhost:5000/tour-types/get-all-tour-types`)
            .then(response => {
                const tourType = response.data.find(tour => tour.name === decodedTourTypeName);
                if (tourType) {
                    setTourType(tourType);
                    setTourTypeId(tourType.id);
                }
            })
            .catch(error => {
    ...
    useEffect(() => {
        setLoading(true);
        if (tourTypeId !== null) {
            axiosInstance.get(`http://localhost:5000/tour-types/${tourTypeId}`)
                .then(response => {
    ...
    useEffect(() => {
        const filteredTours = tours.filter((tour) => {
            ...
            const departureDayCondition = !selectedDepartureDay || formatDepartureDate(tour.departureDay) === selectedDepartureDay;
    ...
    console.log(selectedDepartureDay);
                                ...
                                {loading ? (
                                    ...
                                    <>
                                        <div className="tour-introduce">
                                            <div className="title"> {tourType && tourType.name} </div>
                                            <div className="introduce"> {tourType && tourType.desc} </div>
                                        </div>
